...
func _process(delta):
	...
	# Always clean up any keys at the front that have already passed,
	# regardless of whether a press happened this frame
	while falling_key_queue.size() > 0 and falling_key_queue.front().has_passed:
		RhythmGameSignals.ResetCombo.emit()
		falling_key_queue.front().queue_free()
		falling_key_queue.pop_front()
 
	if falling_key_queue.size() > 0 and Input.is_action_just_pressed(key_name):
		# find the key closest to pass_threshold, ignoring already-missed keys
		var closest = null
		for key in falling_key_queue:
			if key.has_passed:
				continue
			if closest == null or abs(key.global_position.y - key.pass_threshold) < abs(closest.global_position.y - closest.pass_threshold):
				closest = key
 
		if closest == null:
			return # nothing valid to hit
		...
		# Only remove the key from the queue/scene if it was actually hit
		if not was_miss:
